...
pub trait AttestationX509Extension {
    ...
    fn parse<'output,'input:'output>(i: &'input [u8]) -> der_parser::error::BerResult<'output,(Self::Output, AttestationMetadata)>;
...
pub(crate) mod android_key_attestation {
    ...
    impl AuthorizationList {
        pub fn parse<'output,'input:'output>(i: &'input [u8]) -> der_parser::error::BerResult<'output,Self> {
    ...
    impl Data {
        pub fn parse<'output,'input:'output>(i: &'input [u8]) -> der_parser::error::BerResult<'output,(Vec<u8>, AttestationMetadata)> {
            use der_parser::{der::*, error::BerError};
            parse_der_container(|i: &[u8], hdr: Header| {
                if hdr.tag() != Tag::Sequence {
                    return Err(nom::Err::Error(BerError::BerTypeError));
